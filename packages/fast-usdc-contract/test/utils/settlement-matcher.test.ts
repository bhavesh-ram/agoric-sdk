...
const exact = test.macro({
  ...
  exec: (t, want: bigint, pending: bigint[], expectRemaining: bigint[]) => {
    ...
    const got = match.matchAndDequeueSettlement(ADDRESS, want);
    t.deepEqual(
...
const greedy = test.macro({
  ...
  exec: (
    ...
    const got = match.matchAndDequeueSettlement(ADDRESS, target);
    t.deepEqual(
      got.map(tx => tx.tx.amount),
      expectMatch,
      'matched set',
...
test('no combination possible', t => {
  ...
  t.deepEqual(match.matchAndDequeueSettlement(ADDRESS, 6n), []);
});

test('address not found returns []', t => {
  const { match } = t.context;
  t.deepEqual(match.matchAndDequeueSettlement('noble1DOESNOTEXIST', 1n), []);
});

test('empty queue returns []', t => {
  const { match } = t.context;
  match.addPendingSettleTx(makePendingTx(10n));
  // drain it
  void match.matchAndDequeueSettlement(ADDRESS, 10n);
...
test('recipients isolated', t => {
  ...
  match.addPendingSettleTx(makePendingTx(100n, A));
  match.addPendingSettleTx(makePendingTx(150n, B));

  t.deepEqual(match.matchAndDequeueSettlement(A, 100n).length, 1);
  t.deepEqual(match.matchAndDequeueSettlement(B, 150n).length, 1);
});

test('target 0 → [] (greedy early-out)', t => {
  const { match } = t.context;
  match.addPendingSettleTx(makePendingTx(10n));
  t.deepEqual(match.matchAndDequeueSettlement(ADDRESS, 0n), []);
});
...
test('empty list key → [] (greedy early-out)', t => {
  ...
  t.deepEqual(match.matchAndDequeueSettlement(EMPTY, 5n), []);
});
