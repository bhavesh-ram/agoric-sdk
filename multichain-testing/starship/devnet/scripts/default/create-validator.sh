...
function cosmos-sdk-version-v50() {
  ...
  json_content='{
    "pubkey": '$($CHAIN_BIN tendermint show-validator $NODE_ARGS --home $CHAIN_DIR)',
  ...
  $CHAIN_BIN tx staking create-validator /validator.json \
    ...
    --output json \
    --gas $GAS \
    --gas-adjustment 1.5 $NODE_ARGS --home $CHAIN_DIR --yes > /validator.log
...
function cosmos-sdk-version-default() {
  ...
  $CHAIN_BIN tx staking create-validator \
    ...
    --gas $GAS \
    --output json \
    --gas-adjustment 1.5 $args $NODE_ARGS --home $CHAIN_DIR --yes > /validator.log
