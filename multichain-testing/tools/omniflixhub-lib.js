...
const trace = makeTracer('OmniflixHub', false);
 ...
 */
export const makeOmniflixHubd = ({ execFileSync }) => {
  /**
   * @param { {
   *       home?: string;
    ...
    const rw = freeze({
      ...
      tx: async (txArgs, { chainId, from, yes }) => {
        ...
        try {
          ...
          trace('omniflixhubd returned;', detail);
          if (detail.code !== 0) {
